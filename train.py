...
parser.add_argument("--type", default="vit", type=str)
...
###
parser.add_argument("--hp_si", type=float, default=5e0)
parser.add_argument("--hp_am", type=float, default=1e0)
###
...
def train_model(model, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val", "test"]:
            ...
            if phase == "val" and epoch_acc > best_val_acc:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                if args.type in ["vrelg"]:
                    best_sigma = model_ft.shared_rel_pos.sigma_param.item()
                    best_ampitude = model_ft.shared_rel_pos.amplitude_param.item()
                elif args.type in ["vrelglmi"]:
                    sigma_list = []
                    amp_list = []
                    for blk in model.blocks:
                        sigma_list.append(blk.attn.rel_pos.sigma_param.item())
                        amp_list.append(blk.attn.rel_pos.amplitude_param.item())
                    sigma_mean = sum(sigma_list) / len(sigma_list)
                    amp_mean = sum(amp_list) / len(amp_list)
                    best_sigma = sigma_mean
                    best_ampitude = amp_mean
        ...
        ###
        if args.type in ["vrelg"]:
            print("Sigma: {:6f}".format(model_ft.shared_rel_pos.sigma_param.item()))
            print(
                "Amplitude: {:6f}".format(
                    model_ft.shared_rel_pos.amplitude_param.item()
                )
            )
        ###
        elif args.type in ["vrelglmi"]:
            ...
            sigma_mean = sum(sigma_list) / len(sigma_list)
            amp_mean = sum(amp_list) / len(amp_list)
            print("Sigma Mean: {:6f}".format(sigma_mean))
            print("Amplitude Mean: {:6f}".format(amp_mean))
    ...
    if args.type in ["vrelg"]:
        print("Final best sigma: {:6f}".format(best_sigma))
        print("Final best amplitude: {:6f}".format(best_ampitude))
    elif args.type in ["vrelglmi"]:
        print("Final best sigma mean: {:6f}".format(best_sigma))
        print("Final best amplitude mean: {:6f}".format(best_ampitude))
...
if args.type in ["vreli"]:  # before putting cuda, initialize Mlp.
    ...
    num_heads = model_ft.blocks[0].attn.num_heads  # 6
    patch_size_H, patch_size_W = model_ft.patch_embed.patch_size  # 16, 16
    img_size = int(args.model_name[-3:])  # 224
    ...
    def forward(self, attn, shared_rel_pos: Optional[torch.Tensor] = None):
        return attn + self.get_bias() + self.get_gaussian()

    def get_gaussian(self) -> torch.Tensor:
        self.relative_coords_table = torch.stack(
            torch.meshgrid([self.relative_coords_H, self.relative_coords_W])
        )  # size 2, 27, 27. re_gr F
        self.Gaussian_wide = (
            self.amplitude_param
            * self.amplitude_param
            * torch.exp(
                -1.0
                * (
                    self.relative_coords_table[0, :, :] ** 2.0
                    + self.relative_coords_table[1, :, :] ** 2.0
                )
                / (2.0 * (self.sigma_param ** 2.0))
            )
        )  # 27, 27
        self.Gaussian_pos = torch.stack(
            [
                self.Gaussian_wide[
                    start_H : start_H + len_patch, start_W : start_W + len_patch
                ].reshape(-1)
                for start_H in range(
                    len_patch - 1, -1, -1
                )  # this order, H W, is correct.
                for start_W in range(len_patch - 1, -1, -1)
            ]
        ).expand(
            1, num_heads, num_all_patches, num_all_patches
        )  # 196, 196 -> 1, 6, 196, 196. In fact, we can use it without expand.
        return self.Gaussian_pos  # GPU ok

    for blk in model_ft.blocks:
        blk.attn.rel_pos.register_buffer(
            "relative_coords_H",
            torch.arange(-(len_patch - 1), len_patch, dtype=torch.float32),
        )  # value -13. ~ 0. ~ 13. re_gr F. GPU ok.
        blk.attn.rel_pos.register_buffer(
            "relative_coords_W",
            torch.arange(-(len_patch - 1), len_patch, dtype=torch.float32),
        )
        blk.attn.rel_pos.sigma_param = torch.nn.Parameter(
            torch.Tensor([args.hp_si])
        )  # by default, it becomes requires_grad True and updated by GD. GPU ok.
        blk.attn.rel_pos.amplitude_param = torch.nn.Parameter(
            torch.Tensor([args.hp_am])
        )

        blk.attn.rel_pos.forward = forward.__get__(blk.attn.rel_pos)
        blk.attn.rel_pos.get_gaussian = get_gaussian.__get__(blk.attn.rel_pos)
    ...
    def get_gaussian(self) -> torch.Tensor:
        ...
        self.Gaussian_wide = torch.stack(
            [
                self.amplitude_param[i]
                * self.amplitude_param[i]
                * torch.exp(
                    -1.0
                    * (
                        self.relative_coords_table[i, 0, :, :] ** 2.0
                        + self.relative_coords_table[i, 1, :, :] ** 2.0
                    )
                    / (2.0 * (self.sigma_param[i] ** 2.0))
                )
        ...
        self.Gaussian_pos = torch.stack(
            [
                ...
                for start_W in range(len_patch - 1, -1, -1)
                # ]).expand(1, num_heads, num_all_patches, num_all_patches
            ],
            dim=1,
        ).expand(
            1, num_heads, num_all_patches, num_all_patches
        )  # dim 1 is correct. 6, 196, 196 -> 1, 6, 196, 196. In fact, we can use it without expand.
        return self.Gaussian_pos  # GPU ok
    ...
    for blk in model_ft.blocks:
        ...
        blk.attn.rel_pos.sigma_param = torch.nn.Parameter(
            torch.Tensor(num_heads * [args.hp_si])
        )  # by default, it becomes requires_grad True and updated by GD. GPU ok.
        blk.attn.rel_pos.amplitude_param = torch.nn.Parameter(
            torch.Tensor(num_heads * [args.hp_am])
        )
...
param_original = [
    param for name, param in model_ft.named_parameters() if "param" not in name
]
param_my = [param for name, param in model_ft.named_parameters() if "param" in name]
...
if args.hp_opt == "sgd":
    ...
    optimizer_ft = optim.SGD(
        [
            {"params": param_original, "weight_decay": args.hp_wd},
            {"params": param_my, "weight_decay": 0.0},
        ],
        lr=args.hp_lr,
        momentum=0.9,
    )
...
if args.hp_opt == "adamw":
    # optimizer_ft = optim.AdamW(model_ft.parameters(), lr=args.hp_lr, weight_decay=args.hp_wd)
